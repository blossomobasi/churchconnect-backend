...
export class TokenService {
    ...
    async generateAuthTokens(user: Pick<User, "id">) {
        ...
        const tokens = {
            accessToken: this.tokenBackend.encodeToken(accessToken),
            refreshToken: this.tokenBackend.encodeToken(refreshToken),
        };
        ...
        await this.prismaService.refreshToken.create({
            data: {
                ...
                userId: user.id,
                expiresAt: refreshToken.exp ? refreshToken.exp : moment().add(30, "days").toDate(),
            },
        });
