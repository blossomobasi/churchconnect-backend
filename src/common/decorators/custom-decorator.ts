...
export function ApiHttpResponse<T>({ status, type, description, isArray }: { status: number | "default" | "1XX" | "2XX" | "3XX" | "4XX" | "5XX"; type: T | string | boolean | number; description?: string; isArray?: boolean }) {
                                ...
                                if (isPrimitive(type)) {
                                    return isArray
                                        ? {
                                              type: "array",
                                              items: {
                                                  type: typeof type === "function" ? type.name.toLowerCase() : String(type).toLowerCase(),
                                              },
                                          }
                                        : {
                                              type: typeof type === "function" ? type.name.toLowerCase() : String(type).toString().toLowerCase(),
                                          };
                                }
                                return isArray
                                    ? {
                                          type: "array",
                                          items: {
                                              $ref: getSchemaPath(type as Type<T>),
                                          },
                                      }
                                    : { $ref: getSchemaPath(type as Type<T>) };
                            })(),
...
export function ApiHttpErrorResponses() {
    return applyDecorators(
        ...
        ApiResponse({
            status: "4XX",
            type: CustomError,
            description: "Bad Request - All 4XX errors",
        }),
