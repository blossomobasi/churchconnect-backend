...
export class AuthController {
    ...
    @ApiHttpResponse({
        status: 200,
        type: AuthTokensDto,
        description: "Refreshes the access token and refresh token",
    })
    @HttpCode(200)
    @Post("refresh-tokens")
    ...
    @ApiHttpResponse({
        status: 200,
        type: AuthenticationResponseDto,
        description: "Gets the session of a user",
    })
    ...
    @ApiHttpResponse({
        status: 200,
        type: Boolean,
        description: "Requests email verification for a user",
    })
    @HttpCode(200)
    ...
    async verifyResetOTP(@Body() passwordResetDto: VerifyResetOTPDto) {
        const result = await this.authService.verifyResetOTP(passwordResetDto);
        return new HttpResponse("Verify OTP", result, HttpStatus.OK);
    }

    @ApiOperation({ summary: "Verify Email Verification OTP" })
    @ApiHttpErrorResponses()
    @ApiBody({ type: VerifyEmailVerificationOTPDto })
    @ApiHttpResponse({
        status: 200,
        type: Boolean,
    ...
    async verifyEmailVerificationOTP(@Body() verifyEmailVerificationOTPDto: VerifyEmailVerificationOTPDto) {
        const result = await this.authService.verifyEmailVerificationOTP(verifyEmailVerificationOTPDto);
        return new HttpResponse("Verify Email Verification OTP", result, HttpStatus.OK);
    }
